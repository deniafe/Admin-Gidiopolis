...
export const EditEvent = () => {
  ...
  const closeModal = async () => {
    ...
    const myModal = Modal.getInstance(document.getElementById("editEvent"))
    setLoading(false)
    return myModal.hide()
  }
  ...
  const updateEventAfterApproval = () => {
    ...
    // Usage example:
    currentEvent &&
      updateEventApprovalStatus(currentEvent.uid, isApproved) // Call the function to update event approval status
        .then((response) => {
  ...
  useEffect(() => {
    if(currentEvent) {
      setIsApproved(currentEvent.isApproved)
    }
  }, [currentEvent]);
